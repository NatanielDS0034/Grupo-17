...
utilizador(nataniel, 20, accao, [jonatan_heit], [will_smith, joseph_gordon_levitt]).  % Corrigido: will_Smith para will_smith e jose_gordon_levitt para joseph_gordon_levitt
...
menu :-
    nl, write('===== MENU DE RECOMENDACAO DE FILMES ====='), nl,
    write('1. Recomendar filmes para um utilizador'), nl,
    write('2. Listar filmes por genero preferido'), nl,
    write('3. Sair'), nl,
...
executar(1) :-
    write('Digite o nome do utilizador (em letras minúsculas e entre aspas): '), nl,
    read(User),
    recomendar_filme(User, Filme),
    format('Filme recomendado: ~w~n', [Filme]),
    fail; menu.

executar(2) :-
    write('Digite o nome do utilizador (em letras minúsculas e entre aspas): '), nl,
    read(User),
    listar_filmes_por_genero(User),
    menu.

executar(3) :-
    write('Saindo... Obrigado por usar o sistema!'), nl.
...
listar_filmes_por_genero(User) :-
    utilizador(User, _, GeneroPref, _, _),
    filme(Filme, GeneroPref, _, _, _, Aval, Class),
    format('~w (Avaliacao: ~1f, Classificacao: ~d)~n', [Filme, Aval, Class]),
    fail.
listar_filmes_por_genero(_).
